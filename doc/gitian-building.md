...
*Setup instructions for a gitian build of MarteX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MarteX executables. It provides a way to be reasonably sure that the
...
- [Building MarteX](#building-martex)
...
Clone the git repositories for martex and gitian and then checkout the martex version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/martex-crypto/martex.git
cd martex
...
Follow the instructions in [doc/release-process.md](release-process.md) in the martex repository
...
To build MarteX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the martex repository.
...
```bash
    ...
    From https://github.com/martex-crypto/martex
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/martex-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/martex-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/martex-build.assert
```
...
[martex/gitian.sigs](https://github.com/martex-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
