MARTEX Core version *3.1.0* is now available from:  <https://github.com/martex-project/martex/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/martex-project/martex/issues>


Mandatory Update
==============

MARTEX Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zMXT Staking is here! zMXT staking will be activated on the morning of the 8th of May 2018. With the release of zMXT staking, there are effectively 2 versions of zMXT, zMXT minted on the 3.0.6 MARTEX wallet or lower, and zMXT minted on MARTEX wallet version or higher. New features in this release will require the use of zMXT v2, zMXT minted on this wallet release 3.1.0 or later. If you currently hold zMXT v1 and wish to take advantage of zMXT staking and deterministic zMXT, you will need to spend the zMXT v1 to yourself and remint zMXT v2.
Note: To find your zMXT version, click the privacy tab, then the zMXT Control button then expand the arrows next to the desired denomination.


### Deterministic zMXT Seed Keys

zMXT is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zMXT outside of the wallet that the zMXT had been minted on. zMXT can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zMXT minting

zMXT minting now only requires 1 further mint (down from 2) to mature. zMXT mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zMXT Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zMXT denomination has been spent or not.



MXT/zMXT Staking and Masternode Rewards
--------------

### MXT, zMXT and Masternode Payment Schedule

To encourage the use of zMXT and increase the MARTEX zerocoin anonymity set, the MARTEX payment schedule has been changed to the following:

If a user staking zMXT wins the reward for their block, the following zMXT reward will be: 
- 3 zMXT (3 x 1 denominations) rewarded to the staker, 2 MXT rewarded to the masternode owner and 1 MXT available for the budget. This is a total block reward of 6 MXT, up from 5.

If a user staking MXT wins the reward, the following amounts will be rewarded: 
- 2 MXT to the MXT staker, 3 MXT to the Masternode owner and 1 MXT available for the budget. This is a total block reward of 6 MXT, up from 5.


### Return change to sender when minting zMXT

Previously, zMXT minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most MXT and return the change from a zMXT mint to that address.
...
The visual layout of the MARTEX Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zMXT minting feature. This is enabled by default and the enablezeromint=0 setting in the martex.conf file will overwrite the GUI option.
-	The percentage of autominted zMXT can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular MXT.
...
### Summary
 ...
 The MARTEX wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the martex.conf file, found in the MARTEX data directory.
- backuppath = <directory / full path>
- zmxtbackuppath = <directory / full path>
...
* If zMXT backup, auto generated name is `wallet-autozmxtbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
MARTEX Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The MARTEX client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running martexd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the MARTEX support group, which often times sees users say "I didn't spend that zMXT". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
Network Layer 2 Changes (Proposals / Budgets / FastX)
--------------

### Monthly Budget Increase

As voted on by the MARTEX masternodes, the monthly budget available to be utilised has been increased to 42,000 MXT / month. This MXT only has the opportunity to be raised once per month (paid to winning proposals) with any unused MXT not created by the blockchain.

### Budget Finalization Fee

The MARTEX finalization fee for successful proposals has now been reduced, this fee is now 5 MXT down from 50 MXT. The total fee outlay for a successful proposal is now a total of 55 MXT.


### FastX Raw Transactions

 When creating a raw transaction, it is now possible to create the transaction as a FastX transaction. See the updated help documentation for the `createrawtransaction` RPC command.
...
-	Enforced v1 zMXT spends to require a security level of 100
-	Updates to zMXT spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the MARTEX-Qt masternodes tab
-	Updated MARTEX-Qt tooltips
...
-	Updated RPC help outputs & removed the deprecated anonsend. 
-	Refactored code
-	Various bug fixes
...
Several critical security flaws in the zerocoin protocol and MARTEX's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zMXT Control dialog in the QT wallet, a user is able to see zMXT marked as version 1 or 2.

zPoS (zMXT staking)
--------------

Once a zMXT has over 200 confirmations it becomes available to stake. Staking zMXT will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zMXT. So for example if a 1,000 zMXT denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zMXTs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within MARTEX's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zMXT, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zMXT that can be recalculated at any time using the seed. Deterministic zMXT allows for users to backup all of their future zMXT by simply recording their seed and keeping it in a safe place (similar to backing up a private key for MXT). The zMXT seed needs to remain in the wallet in order to spend the zMXT after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zMXT data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

MARTEX's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which MARTEX used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to MARTEX using the bug bounty program. The modulus is now correctly imported and MARTEX's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zMXT v2: zPoS dzMXT ezMXT external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zMXT balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zMXT status output (Mrs-X)
 - #499 `6305264f2` [Qt] Send popup simplified + FastTX -> FastX (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zMxt Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault martex-cli getinfo while loading block index (Mrs-X)
 ...
 - #423 `5dfb48ee6` [RPC] Add FastX to rpcrawtransaction. (presstab)
 - #170 `027f16c64` [RPC] Convert source tree from json_spirit to UniValue (Fuzzbawls)

### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zMXT spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zMXT. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zMXT. (presstab)
 
### Miscellaneous
 - #559 `d2b017217` [Bug] Segfault with -enablefasttx=0 / -enablefasttx=false (Mrs-X)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/martex-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
