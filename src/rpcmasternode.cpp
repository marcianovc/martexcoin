...
{
    throw runtime_error("AnonSend is not supported any more. Use Zerocoin\n");
    
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "anonsend <martexaddress> <amount>\n"
            "martexaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "anonsend <martexaddress> <amount>\n"
            "martexaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) MarteX address of current masternode\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in martex.conf\n"
    ...
    BOOST_FOREACH (PAIRTYPE(int, CMasternode) & s, vMasternodeRanks) {
        ...
        if (mn != NULL) {
            if (strFilter != "" && strTxHash.find(strFilter) == string::npos &&
                ...
                CMarteXAddress(mn->pubKeyCollateralAddress.GetID()).ToString().find(strFilter) == string::npos) continue;
            ...
            obj.push_back(Pair("addr", CMarteXAddress(mn->pubKeyCollateralAddress.GetID()).ToString()));
...
Value getmasternodecount (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() > 0))
        throw runtime_error(
            ...
            "  \"obfcompat\": n,    (numeric) AnonSend Compatible\n"
...
Value masternodecurrent (const Array& params, bool fHelp)
{
    ...
    if (winner) {
        ...
        obj.push_back(Pair("pubkey", CMarteXAddress(winner->pubKeyCollateralAddress.GetID()).ToString()));
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) MarteX address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
    ...
    if (pmn) {
        ...
        mnObj.push_back(Pair("addr", CMarteXAddress(pmn->pubKeyCollateralAddress.GetID()).ToString()));
...
Value getmasternodewinners (const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) MarteX MN Address\n"
