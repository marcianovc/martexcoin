...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    return CMarteXAddress(keyID).ToString();
}


CMarteXAddress GetAccountAddress(string strAccount, bool bForceNew = false)
{
    ...
    return CMarteXAddress(account.vchPubKey.GetID());
}
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH (const PAIRTYPE(CMarteXAddress, CAddressBookData) & item, pwalletMain->mapAddressBook) {
        const CMarteXAddress& address = item.first;
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH (set<CTxDestination> grouping, pwalletMain->GetAddressGroupings()) {
        ...
        BOOST_FOREACH (CTxDestination address, grouping) {
            Array addressInfo;
            addressInfo.push_back(CMarteXAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CMarteXAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CMarteXAddress(address).Get())->second.name);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    CMarteXAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    // martex address
    CMarteXAddress address = CMarteXAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MarteX address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    string strAccount = AccountFromValue(params[0]);
    CMarteXAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MarteX address");
    CAmount nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "    {\n"
                                        "      \"address\":amount   (numeric) The martex address is the key, the numeric amount in btc is the value\n"
                                        "      ,...\n"
                                        "    }\n"
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return CMarteXAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CMarteXAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH (const PAIRTYPE(CMarteXAddress, CAddressBookData) & item, pwalletMain->mapAddressBook) {
        ...
        map<CMarteXAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending MARTEXs\n"
            "\nArguments:\n"
            ...
            "\nUnlock the wallet for 60 seconds but allow AnonSend only\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\" 60 true") +
    ...
    if (!pwalletMain->Unlock(strWalletPass, anonymizeOnly))
        throw JSONRPCError(RPC_WALLET_PASSMARTEXASE_INCORRECT, "Error: The wallet passphrase entered was incorrect.");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending MARTEXs\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
...
Value lockunspent(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending MARTEXs.\n"
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in MARTEX/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getwalletinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total MARTEX balance of the wallet\n"
...
Array printAddresses()
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        std::string strAdd = CMarteXAddress(utxoAddress).ToString();
...
Value multisend(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 1) {
        ...
        if (strCommand == "print") {
            ...
            if (CMarteXAddress(pwalletMain->vMultiSend[0].first).IsValid()) {
    ...
    if (params.size() == 2 && params[0].get_str() == "disable") {
        ...
        if (!CMarteXAddress(disAddress).IsValid())
    ...
    string strAddress = params[0].get_str();
    CMarteXAddress address(strAddress);
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MARTEX address");
...
Value mintzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "mintzerocoin <amount>\n"
            "Usage: Enter an amount of MarteX to convert to zMARTEX"
            + HelpRequiringPassphrase());

    int64_t nTime = GetTimeMillis();

    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zMARTEX is currently disabled due to maintenance.");
...
Value spendzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zMARTEX (zerocoins) into MARTEX. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over MARTEX (change), the wallet can convert it automatically back to zerocoins [true]\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zMARTEX
    bool fMinimizeChange = params[2].get_bool();    // Minimize change
    int nSecurityLevel = params[3].get_int();       // Security level

    CMarteXAddress address = CMarteXAddress(); // Optional sending address. Dummy initialization here.
    if (params.size() == 5) {
        ...
        address = CMarteXAddress(params[4].get_str());
        if(!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid MarteX address");
    }
    ...
    for (unsigned int i = 0; i < wtx.vout.size(); i++) {
        ...
        if(txout.scriptPubKey.IsZerocoinMint())
            ...
            out.push_back(Pair("address", CMarteXAddress(dest).ToString()));
        vout.push_back(out);
    }
    ...
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
            "exportzerocoins include_spent ( denomination )\n"
                ...
                "2. \"denomination\"         (integer, optional) Export a specific denomination of zMARTEX\n"
    ...
    if(fHelp || params.size() == 0)
        throw runtime_error(
            "importzerocoins importdata \n"
                ...
                "\"value\"            (string) the total zMARTEX value of zerocoin mints that were added\n"
...
Value reconsiderzerocoins(const Array& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "reconsiderzerocoins\n"
                "\nCheck archived zMARTEX list to see if any mints were added to the blockchain.\n"
