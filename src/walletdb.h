...
public:
    ...
    bool WriteZMXTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMXTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMXTSeed_deprecated(uint256& seed);
    bool EraseZMXTSeed();
    bool EraseZMXTSeed_deprecated();

    bool WriteZMXTCount(const uint32_t& nCount);
    bool ReadZMXTCount(uint32_t& nCount);
