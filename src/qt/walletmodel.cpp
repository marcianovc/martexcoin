...
{
    CMarteXAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered martex address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CMarteXAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        if (recipients[0].useFastTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("FastTX doesn't support sending values that high yet. Transactions are currently limited to %1 MARTEX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useFastTX);
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal martex:URI (martex:XyZ...?message=example)
            {
        ...
        if (!wallet->CommitTransaction(*newTx, *keyChange, (recipients[0].useFastTX) ? "ix" : "tx"))
...
bool WalletModel::setWalletLocked(bool locked, const SecureString& passMarteXase, bool anonymizeOnly)
{
    if (locked) {
        // Lock
        ...
        // Unlock
        return wallet->Unlock(passMarteXase, anonymizeOnly);
    }
}
...
{
    QString strAddress = QString::fromStdString(CMarteXAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CMarteXAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CMarteXAddress(sAddress).Get();
...
bool WalletModel::isMine(CMarteXAddress address)
{
