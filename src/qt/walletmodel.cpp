...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useFastTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("FastX doesn't support sending values that high yet. Transactions are currently limited to %1 MXT.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzMXTReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzMXTReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzMXTReset.connect(boost::bind(NotifyzMXTReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzMXTReset.disconnect(boost::bind(NotifyzMXTReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zmxtTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
