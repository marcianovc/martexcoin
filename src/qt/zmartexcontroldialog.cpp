...
#include "ui_zmartexcontroldialog.h"
...
std::list<std::string> ZMarteXControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMarteXControlDialog::listMints;

ZMarteXControlDialog::ZMarteXControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMarteXControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMarteXControlDialog::~ZMarteXControlDialog()
{
    delete ui;
}

void ZMarteXControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMarteXControlDialog::updateList()
{
...
void ZMarteXControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMarteXControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMarteX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMarteXControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMarteXControlDialog::GetSelectedMints()
{
...
void ZMarteXControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
