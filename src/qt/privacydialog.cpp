...
#include "zmartexcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMARTEX ought to be enough for anybody." - Bill Gates, 2017
    ui->zMARTEXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMARTEXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMARTEX->setEnabled(false);
        ui->pushButtonMintzMARTEX->setToolTip(tr("zMARTEX is currently disabled due to maintenance."));

        ui->pushButtonSpendzMARTEX->setEnabled(false);
        ui->pushButtonSpendzMARTEX->setToolTip(tr("zMARTEX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMARTEXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMARTEX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMARTEX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMARTEX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMARTEX in ") + 
...
void PrivacyDialog::on_pushButtonSpendzMARTEX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMARTEX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMARTEX
        sendzMARTEX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMARTEX
    sendzMARTEX();
}

void PrivacyDialog::on_pushButtonZMarteXControl_clicked()
{
    ZMarteXControlDialog* zMARTEXControl = new ZMarteXControlDialog(this);
    zMARTEXControl->setModel(walletModel);
    zMARTEXControl->exec();
}

void PrivacyDialog::setZMarteXControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMARTEXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMARTEX()
{
    QSettings settings;

    // Handle 'Pay To' address options
    CMarteXAddress address(ui->payTo->text().toStdString());
    ...
    double dAmount = ui->zMARTEXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMARTEX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MARTEX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMARTEX</b>";
    ...
    // use mints from zMARTEX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMarteXControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMarteXControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmartex selector in case it was used
    ZMarteXControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMARTEX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " MarteX, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMARTEX Mint");
        else if(ExtractDestination(txout.scriptPubKey, dest))
            strStats += tr(CMarteXAddress(dest).ToString().c_str());
        strStats += "\n";
    }
    double fDuration = (double)(GetTimeMillis() - nTime)/1000.0;
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMARTEX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMARTEX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMARTEX "));
    ui->labelzMARTEXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMARTEXSyncStatus->setVisible(fShow);
}
