...
#include "ui_zmxtcontroldialog.h"
...
std::set<std::string> ZMxtControlDialog::setSelectedMints;
std::set<CMintMeta> ZMxtControlDialog::setMints;

ZMxtControlDialog::ZMxtControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMxtControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMxtControlDialog::~ZMxtControlDialog()
{
    delete ui;
}

void ZMxtControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMxtControlDialog::updateList()
{
...
void ZMxtControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMxtControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMxt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMxtControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMxtControlDialog::GetSelectedMints()
{
...
void ZMxtControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
