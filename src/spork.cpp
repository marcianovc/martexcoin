...
// MARTEX: on startup load spork values from previous session if they exist in the sporkDB
...
bool CSporkManager::CheckSignature(CSporkMessage& spork, bool fCheckSigner)
{
    ...
    bool fValidWithNewKey = AnonSendSigner.VerifyMessage(pubkeynew, spork.vchSig,strMessage, errorMessage);
    ...
    if (!fValidWithNewKey && GetAdjustedTime() < Params().RejectOldSporkKey()) {
        ...
        return AnonSendSigner.VerifyMessage(pubkeyold, spork.vchSig, strMessage, errorMessage);
    }
