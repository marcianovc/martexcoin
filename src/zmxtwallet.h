...
#ifndef MARTEX_ZMXTWALLET_H
#define MARTEX_ZMXTWALLET_H
...
public:
    CzMXTWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMXT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMXT(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MARTEX_ZMXTWALLET_H
