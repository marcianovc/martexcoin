...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 MXT finalization collateral
            //       will also cover the old 50 MXT finalization collateral.
    ...
    */
    ...
    //if we're syncing we won't have fastTX information, so accept 1 confirmation
...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    if (!mapCollateralTxids.count(tempBudget.GetHash())) {
        ...
        // Send the tx to the network. Do NOT use FastTx, locking might need too much time to propagate, especially for testnet
