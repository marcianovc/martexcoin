...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        if (pnode->fAnonSendMaster) {
            if (AnonSendPool.pSubmittedToMasternode != NULL && pnode->addr == AnonSendPool.pSubmittedToMasternode->addr) continue;
            LogPrint("masternode","Closing Masternode connection peer=%i \n", pnode->GetId());
            pnode->fAnonSendMaster = false;
            pnode->Release();
        }
    }
}
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (!AnonSendSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubKeyCollateralAddress)) {
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
    else if (strCommand == "dsee") { //AnonSend Election Entry
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 MARTEX tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 MXT tx -> 1 confirmation
