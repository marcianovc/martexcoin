// Copyright (c) 2018 The MARTEX developers
...
CzMXTTracker::CzMXTTracker(std::string strWalletFile)
{
...
CzMXTTracker::~CzMXTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMXTTracker::Init()
{
...
bool CzMXTTracker::Archive(CMintMeta& meta)
{
...
bool CzMXTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMXTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMXTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMXTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMXTTracker::GetSerialHashes()
{
...
CAmount CzMXTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMXTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMXTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMXTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMXTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMXTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMXTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMXTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMXTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMXTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMXTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMXTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMXTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMXTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMXTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMXTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMXTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmxt from DB\n", __func__, listDeterministicDB.size());
    }
