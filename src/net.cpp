...
CNode* ConnectNode(CAddress addrConnect, const char* pszDest, bool AnonSendMaster)
{
    if (pszDest == NULL) {
        ...
        if (IsLocal(addrConnect) && !AnonSendMaster)
            return NULL;

        // Look for an existing connection
        CNode* pnode = FindNode((CService)addrConnect);
        if (pnode) {
            pnode->fAnonSendMaster = AnonSendMaster;
    ...
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, Params().GetDefaultPort(), nConnectTimeout, &proxyConnectionFailed) :
        ...
        if (AnonSendMaster) pnode->fAnonSendMaster = true;
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (MARTEX users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "MARTEX " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. MARTEX Core is probably already running."), addrBind.ToString());
        else
