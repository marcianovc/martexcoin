...
CzMXTWallet::CzMXTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmxt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMXTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMXTSeed_deprecated()) {
                    LogPrintf("%s: Updated zMXT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmxt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmxt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMXTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMXTCount(nCountLastUsed);
    else if (!walletdb.ReadZMXTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMXTWallet::Lock()
{
    seedMaster = 0;
}

void CzMXTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMXTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMXT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMXTWallet::LoadMintPoolFromDB()
{
...
void CzMXTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMXTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMXTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmxtTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMXTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMXT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmxtTracker which also adds to database
    pwalletMain->zmxtTracker->Add(dMint, true);
...
void CzMXTWallet::SeedToZMXT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMXTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMXTWallet::GenerateDeterministicZMXT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMXTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMXTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
