...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MARTEX at the same time
    ONLY_5000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMARTEX send
enum ZerocoinSpendStatus {
    ZMARTEX_SPEND_OKAY = 0,                            // No error
    ZMARTEX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMARTEX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMARTEX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMARTEX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMARTEX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMARTEX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMARTEX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMARTEX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMARTEX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMARTEX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMARTEX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMARTEX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMARTEX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMARTEX_SPENT_USED_ZMARTEX = 14,                     // Coin has already been spend
    ZMARTEX_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    bool SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nAnonSendRoundsMin, int nAnonSendRoundsMax) const;
    bool SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nAnonSendRoundsMin, int nAnonSendRoundsMax);
    ...
    bool CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CMarteXAddress* address = NULL);
    ...
    bool SpendZerocoin(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CMarteXAddress* addressTo = NULL);
    std::string ResetMintZerocoin(bool fExtendedSearch);
    std::string ResetSpentZerocoin();
    void ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored);
    void ZMarteXBackupWallet();
    ...
    */
    ...
    void setZMarteXAutoBackups(bool fEnabled)
    {
    ...
    std::map<CMarteXAddress, std::vector<COutput> > AvailableCoinsByAddress(bool fConfirmed = true, CAmount maxCoinValue = 0);
...
public:
    ...
    //Used with AnonSend. Will return largest nondenom, then denominations, then very small inputs
    int Priority() const
    {
