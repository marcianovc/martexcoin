...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MXT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMXT send
enum ZerocoinSpendStatus {
    ZMxt_SPEND_OKAY = 0,                            // No error
    ZMxt_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMxt_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMxt_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMxt_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMxt_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMxt_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMxt_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMxt_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMxt_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMxt_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMxt_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMxt_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMxt_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMxt_SPENT_USED_ZMxt = 14,                      // Coin has already been spend
    ZMxt_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMxt_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZMxtBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMxtOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmxtAuto) const;
    ...
    */
    ...
    CzMXTWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMXTTracker> zmxtTracker;
    ...
    void setZWallet(CzMXTWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmxtTracker = std::unique_ptr<CzMXTTracker>(new CzMXTTracker(strWalletFile));
    }

    CzMXTWallet* getZWallet() { return zwalletMain; }
    ...
    void setZMxtAutoBackups(bool fEnabled)
    {
    ...
    /** zMXT reset */
    boost::signals2::signal<void()> NotifyzMXTReset;
